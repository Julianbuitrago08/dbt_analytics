...
WITH paid_orders as (select Orders.ID as order_id,
    ...
    FROM {{ref('bronze_jaffashop_orders')}} as Orders
    left join ( select 
                    ORDERID as order_id, max(CREATED) as payment_finalized_date, 
                    {{ cents_to_dollars('coalesce(sum(AMOUNT), 0)') }} as total_amount_paid
                from {{ref('bronze_jaffashop_payment')}}
                where payment_status <> 'fail'
                group by 1) p 
        ON orders.ID = p.order_id
left join {{ref('bronze_jaffashop_customers')}} C on orders.USER_ID = C.ID )
,
customer_orders 
    ...
    from {{ref('bronze_jaffashop_customers')}} C 
    left join {{ref('bronze_jaffashop_orders')}} as Orders
